<template>
  <div class="app-container">
    ...
    <el-table :key='tableKey' :data="specificationModelTable" v-loading="listLoading" height="705px" border fit highlight-current-row style="width: 100%;" @selection-change='selectRow' ref="moviesTable">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="480px">
      <el-form :rules="rules" status-icon ref="dataForm" :model="temp" label-position="right" label-width="120px" style='margin-left: 25px;' :disabled="setDisabled">
        ...
        <el-form-item label="规格型号名称" required prop="name">
          <el-input class="filter-item" v-model="temp.name" style="width:250px" :maxlength="15"></el-input>
        </el-form-item>
        ...
        <el-form-item :label="'设备小类'" required prop="sbtypeThree">
...
export default {
  ...
  data() {
    const validateName = (rule, value, callback) => { // 规格型号名称验证
      if (value === '') {
        callback(new Error('请输入规格型号名称'))
      } else {
        callback()
      }
    }
