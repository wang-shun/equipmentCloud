<template>
  <div class="app-container">
    ...
    <el-table :key='tableKey' :data="eqApplicationTable" v-loading="listLoading" border fit highlight-current-row :height="tableHeight"
      style="width: 100%" @selection-change='selectRow' ref="moviesTable">
      <el-table-column type="selection" width="35"></el-table-column>
      <el-table-column width="200px" align="center" label="领用单位">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="80%">
      ...
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="right" label-width="100px" :disabled="setDisabled">
        ...
        <el-form-item label="领用日期" prop="applyDate" class="el-col el-col-6">
          <el-date-picker v-model="temp.applyDate" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" class="filter-item" :readonly="!(textMap[dialogStatus] === '通用设备领用申请')"></el-date-picker>
        </el-form-item>
    ...
    <el-dialog width="1000px" :title="textMap_eq[dialogStatus_eq]" :visible.sync="equipmentDialog" append-to-body>
      <el-form ref="eqDataForm" :model="eqColumn" label-position="right" label-width="100px">
        ...
        <el-form-item class="el-col el-col-8" style="text-align: right;">
          ...
          <el-button class="filter-item" size="small" v-waves icon="el-icon-refresh" @click="resetForm()">重置</el-button>
        </el-form-item>
        <div style="clear:both;"></div>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getEqSmallTypeList(event) { // 中类联动设备小类
      this.eqColumn.smallTypeCode = '' // 清空联动的下拉框默认值
    ...
    searchEq() { // 搜索设备调用的方法
      ...
      const month = this.temp.applyDate.substring(5, 7)
      const param = {
        equipmentType: '1', // 通用设备
        positionId: this.temp.planPosition, // 使用单位id暂时写死
        bigTypeCode: this.eqColumn.bigTypeCode, // 大类编码
    ...
    getList(page, size) { // 设备领用列表
      this.listLoading = true
      const _this = this
      require_get('/v1/elUses?page=' + page + '&size=' + size + '&jsonString={"useEquipmentType":"1"}', {
        token: token,
        authorityCode: 1
      }, {}, function(res) {
        _this.listLoading = false
    ...
    handleCreate() { // 设备申请弹窗
      this.temp.useAt = this.formatDate()
      ...
      require_get('/v1/elUses/findById/' + id, {
        ...
        if (res.data.status === 200) {
          ...
          result.elUseItems.forEach((val, idx) => {
            ...
            obj.modelName = val.sbEquipmentT.sbmodelName // 规格型号编码
