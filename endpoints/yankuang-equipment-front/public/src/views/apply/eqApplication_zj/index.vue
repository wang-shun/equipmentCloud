<template>
  <div class="app-container">
    <div class="filter-container">
      <el-row>
        <el-input @keyup.enter.native="search" style="width: 140px;" class="filter-item" placeholder="领用人" v-model="listQuery.name"></el-input>
        <el-select filterable clearable style="width: 140px" class="filter-item" v-model="listQuery.planPosition" placeholder="领用单位">
          <el-option v-for="item in applyCompanyList" :key="item.positionId" :label="item.name" :value="item.positionId">
          </el-option>
        </el-select>
        <el-date-picker v-model="listQuery.startDate" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" style="width: 140px;" class="filter-item" placeholder="领用日期起"></el-date-picker>
        <el-date-picker v-model="listQuery.endDate" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" style="width: 140px;" class="filter-item" placeholder="领用日期止"></el-date-picker>
      </el-row>
      <el-row>
        ...
        <div class="btn_right"  v-for='item in authorityCodebutton' :key='item.a'>
          <el-button class="filter-item" size="small" style="margin-left: 10px;" type="primary" v-waves icon="el-icon-circle-plus-outline" v-if="item.name === '领用申请'" @click="handleCreate">领用申请</el-button>
    ...
    <!--设备申请列表-->
    <el-table :key='tableKey' :data="eqApplicationTable" v-loading="listLoading" border fit highlight-current-row :height="tableHeight"
      style="width: 100%;max-height:740px;" @selection-change='selectRow' ref="moviesTable">
      ...
      <el-table-column width="180px" align="center" label="领用单位">
        <template slot-scope="scope">
          <span class="link-type" @click="handleInfo(scope.row)">{{scope.row.name}}</span>
        </template>
      </el-table-column>
      <el-table-column width="150px" align="center" label="领用人">
        <template slot-scope="scope">
          <span>{{scope.row.userName}}</span>
        </template>
      </el-table-column>
      <el-table-column width="180px" align="center" label="领用日期">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="80%">
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="100px" :disabled="setDisabled">
        ...
        <el-form-item label="领用单位" required prop="planPosition" class="el-col el-col-6">
          <el-select filterable class="filter-item" v-model="temp.planPosition">
            <el-option v-for="item in applyCompanyList" :key="item.positionId" :label="item.name" :value="item.positionId">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="领用人" prop="applyName" class="el-col el-col-6">
          <span>{{temp.applyName}}</span>
        </el-form-item>
        <el-form-item label="领用日期" prop="applyDate" class="el-col el-col-6">
          <el-date-picker v-model="temp.applyDate" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" class="filter-item" :readonly="!(textMap[dialogStatus] === '综机设备领用申请')"></el-date-picker>
        </el-form-item>
        <el-form-item label="计费开始日期" prop="useAt" class="el-col el-col-6">
          <el-date-picker v-model="temp.useAt" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" class="filter-item" :readonly="!(textMap[dialogStatus] === '通用设备领用申请')"></el-date-picker>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      rules: { // 校验
        planPosition: [{ required: true, message: '请选择领用单位', trigger: 'change' }]
      },
  ...
  methods: {
      ...
      require_get('/v1/elplan?page=1&size=1000&type=3&pCode', {
        ...
        if (res.data.status === 200) {
          ...
          list.forEach((val, idx) => {
            ...
            // console.log(_this.applyCompanyList)
          })
        } else {
          _this.$notify({
            title: '错误',
    ...
    search() { // 搜索领用计划调用的方法
      ...
      jsonString.useEquipmentType = '2'
      require_get('/v1/elUses?page=' + page + '&size=' + size + '&jsonString=' + JSON.stringify(jsonString), {
        token: token,
        authorityCode: 1
      }, {}, function(res) {
    ...
    searchEq() { // 搜索设备调用的方法
      ...
      const param = {
        equipmentType: '2', // 综机设备
        positionId: this.temp.planPosition, // 使用单位id暂时写死
        bigTypeCode: '99', // 大类编码 综机只能搜99
        middleTypeCode: this.eqColumn.middleTypeCode,
        smallTypeCode: this.eqColumn.smallTypeCode,
        equipmentCode: this.eqColumn.sbsbm,
        equipmentName: this.eqColumn.equipmentName,
        equipmentModel: this.eqColumn.equipmentSpecification,
        // equipmentFactory: this.eqColumn.develop,
        equipmentFactory: '',
        effectCode: this.eqColumn.effectCode,
        centerYear: year // 申请年限
      }
      require_post('/v1/elplan/findElPlanUse', {
        token: token,
        authorityCode: 1
      }, param, function(res) {
        if (res.data.status === 200) {
    ...
    getList(page, size) { // 设备领用列表
      this.listLoading = true
      const _this = this
      require_get('/v1/elUses?page=' + page + '&size=' + size + '&jsonString={"useEquipmentType":"2"}', { // 获取综机
        token: token,
        authorityCode: 1
      }, {}, function(res) {
        _this.listLoading = false
      ...
      require_get('/v1/elUses/findById/' + id, {
        ...
        if (res.data.status === 200) {
          ...
          result.elUseItems.forEach((val, idx) => {
            const obj = {}
            obj.teachCode = val.sbEquipmentZ.techCode // 技术识别号
            obj.effectName = val.equipmentEffect // 功能位置
            obj.bigTypeName = val.sbEquipmentZ.sbtypeOneName // 大类编码
            obj.middleTypeName = val.sbEquipmentZ.sbtypeTwoName // 中类编码
            obj.smallTypeName = val.sbEquipmentZ.sbtypeThreeName // 小类编码
            obj.modelName = val.sbEquipmentZ.sbmodelName // 规格型号编码
            obj.mainParamName = val.mainParamName // 主要技术参数名
            obj.mainParamValue = val.mainParamValue // 主要技术参数值
            obj.equipmentName = val.sbEquipmentZ.name // 设备名称
            obj.equipmentNum = val.equipmentNum // 设备数量
            obj.equipmentCode = val.sbEquipmentZ.code // 设备识别码
            obj.status = '备用' // 备注
    ...
    addApply() { // 添加设备申请
      this.$refs['dataForm'].validate((valid) => {
        ...
        if (valid) {
          ...
          const params = {
            rentAt: this.temp.applyDate,
            useAt: this.temp.useAt,
            ...
            useEquipmentType: 2, // 不加默认通用，2为综机
            elUseItems: elUseItemsArr
          }
          if (elUseItemsArr.length !== 0) {
            require_post('/v1/elUses', {
              token: token,
              authorityCode: 1
            }, params, function(res) {
              if (res.data.status === 200) {
      ...
      if (this.selectlistRow) {
        if (this.selectlistRow.length !== 0) {
          for (let i = 0; i < this.selectlistRow.length; i++) {
            ...
            obj.teachCode = this.selectlistRow[i].teachCode // 技术识别号
            obj.effectName = this.selectlistRow[i].effectName // 功能位置编码
            ...
            obj.modelName = this.selectlistRow[i].modelName // 规格型号编码
            obj.mainParamName = this.selectlistRow[i].mainParamName // 主要技术参数名
            obj.mainParamValue = this.selectlistRow[i].mainParamValue // 主要技术参数值
            ...
            obj.status = this.selectlistRow[i].status // 备注
            obj.equipmentId = this.selectlistRow[i].equipmentId
            obj.planUseId = this.selectlistRow[i].planUseId
    ...
    examinePass() { // 审核通过
      ...
      require_put('/v1/elUses/successStatus', {
        token: token,
        authorityCode: 1
      }, params, function(res) {
        if (res.data.status === 200) {
