    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="80%">
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="100px" :disabled="setDisabled">
        ...
        <el-form-item label="退租日期" prop="applyDate" class="el-col el-col-6">
          <el-date-picker v-model="temp.applyDate" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" class="filter-item" :readonly="!(textMap[dialogStatus] === '综机设备退租申请')"></el-date-picker>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    searchEq() { // 搜索设备调用的方法
      ...
      const jsonString = {}
      jsonString.planEquipmentType = '2' // 综机设备
      jsonString.bigTypeCode = '99' // 大类编码 综机设备只能搜99
      ...
      // jsonString.equipmentFactory = this.eqColumn.develop
      jsonString.equipmentFactory = ''
    ...
    getList(page, size) { // 设备领用列表
      this.listLoading = true
      const _this = this
      require_get('/v1/elUses/findListTz?page=' + page + '&size=' + size + '&jsonString={"useEquipmentType":"2"}', {
        token: token,
        authorityCode: 1
      }, {}, function(res) {
        _this.listLoading = false
      ...
      require_get('/v1/elUses/findById/' + id, {
        ...
        if (res.data.status === 200) {
          ...
          result.elUseItems.forEach((val, idx) => {
            ...
            obj.equipmentModel = val.sbEquipmentZ.sbmodelCode // 规格型号
