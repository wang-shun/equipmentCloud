<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input @keyup.enter.native="search" style="width: 140px;" class="filter-item" placeholder="编号" v-model="listQuery.code"></el-input>
      <el-input @keyup.enter.native="search" style="width: 140px;" class="filter-item" placeholder="规格型号名称"  v-model="listQuery.name"></el-input>
      ...
      <div class="btn_right" v-for='item in authorityCodebutton' :key='item.a'>
        <el-button class="filter-item btn_right" size="small" style="margin-left: 10px;" type="danger" v-waves icon="el-icon-delete" v-if="item.name === '删除'" @click.prevent="delSpModal()">{{item.name}}</el-button>
        <el-button class="filter-item btn_right" size="small" style="margin-left: 10px;" type="primary" v-waves icon="el-icon-edit" v-if="item.name === '编辑'" @click="edit()">{{item.name}}</el-button>
        <el-button class="filter-item btn_right" size="small" style="margin-left: 10px;" type="primary" v-waves icon="el-icon-circle-plus-outline" v-if="item.name === '添加'" @click="handleCreate()">{{item.name}}</el-button>
      </div>
    </div>
    ...
    <el-table :key='tableKey' :data="specificationModelTable" v-loading="listLoading" :height="tableHeight" border fit highlight-current-row style="width: 100%;" @selection-change='selectRow' ref="moviesTable">
      ...
      <el-table-column width="200px" align="center" :label="'规格型号名称'">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="480px">
      <el-form :rules="rules" status-icon ref="dataForm" :model="temp" label-position="right" label-width="120px" style='margin-left: 25px;' :disabled="setDisabled">
        ...
        <el-form-item :label="'设备大类'" required prop="sbtypeOne">
          <el-select filterable class="filter-item" v-model="temp.sbtypeOne" v-on:change="getEqMiddleTypeList($event)" style="width:250px">
        ...
        <el-form-item :label="'设备中类'" required prop="sbtypeTwo">
          <el-select filterable class="filter-item" v-model="temp.sbtypeTwo" v-on:change="getEqSmallTypeList($event)" style="width:250px">
        ...
        <el-form-item :label="'设备小类'" prop="sbtypeThree">
          <el-select filterable class="filter-item" v-model="temp.sbtypeThree" style="width:250px">
      ...
      <div slot="footer" class="dialog-footer" v-if="isUpdate">
        ...
        <el-button v-if="dialogStatus=='create'" type="primary" @click="addSpModel" :loading='istianjia'>确定</el-button>
      ...
      if (this.selectlistRow) {
        if (this.selectlistRow.length !== 0) {
          this.$confirm('确认要删除么?', '提示', {
            ...
            require_delete('/v1/sbmodel/deletes/' + ids, {
              token: token,
              authorityCode: 1
            }, {}, function(res) {
              if (res.data.meta.code === '200') {
