<template>
  <div class="app-container" >
    <div class="filter-container" >
      <el-input @keyup.enter.native="search" class="filter-item" placeholder="存货号" style="width: 135px;" v-model="listQuery.code"></el-input>
    ...
    <el-table :key='tableKey' :data="tableData3" stripe border fit highlight-current-row v-loading="listLoading" :height="tableHeight" style="width: 100%;" @selection-change='selectRow' ref="moviesTable">
      ...
      <el-table-column width="150px" label="存货号" align="center">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="1100px" style='margin-top: 1% !important;padding: 10px 20px;'>
      <el-form class="form" :rules="rules" status-icon ref="dataForm" :model="temp" label-position="right" label-width="130px" style='max-height:550px;overflow:auto;' :disabled="setDisabled">
        <el-col :span="8">
          <el-form-item :label="'存货号:'" :disable='disable' required prop='code'>
        ...
        <el-col :span="8">
          <el-form-item :label="'小类:'" prop="sbtypeThree">
        ...
        <el-col :span="8">
          <el-form-item :label="'规格型号:'" prop="sbmodelCode">
        ...
        <el-col :span="8">
          <el-form-item :label="'数量:'" required prop='assetNum'>
            <el-select filterable v-model="temp.assetNum" placeholder="请选择">
              <el-option
                v-for="item in isassetNumList"
                :key="item.value"
                :label="item.label"
                :value="item.value">
              </el-option>
        ...
        <el-col :span="8">
          <el-form-item :label="'单位:'" required prop="assetUnit">
            <el-select filterable v-model="temp.assetUnit" placeholder="请选择">
              <el-option
                v-for="item in isassetUnitList"
                :key="item.value"
                :label="item.label"
                :value="item.value">
              </el-option>
        ...
        <el-col :span="8">
          <el-form-item :label="'技术识别码:'" prop='techCode'>
        ...
        <el-col :span="8">
          <el-form-item :label="'仓库:'" prop='ware'>
        ...
        <el-col :span="8">
          <el-form-item :label="'是否开启rfid:'" prop='isRfid'>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        code: [{ msg: '请输入存货号', validator: validateRquNumEn }],
        ...
        techCode: [{ validator: validateNumEn }],
        ...
        costYears: [{ required: true, msg: '请输入折旧年限', validator: validateInt }],
        ...
        assetNum: [{ required: true, message: '请选择数量', trigger: 'change' }],
        assetUnit: [{ required: true, message: '请选择单位', trigger: 'change' }]
      },
  ...
  methods: {
    ...
    getList(page, size) {
      ...
      require_get('/v1/sbequipmentz/list', {
        token: token,
        authorityCode: 1
      }, params, function(res) {
        // setTimeout(() => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          this.disable = true
          const _this = this
          require_post('/v1/sbequipmentz/create', {
            token: token,
            authorityCode: 1
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          const _this = this
          require_put('/v1/sbequipmentz/update', {
            token: token,
            authorityCode: 1
          }, _this.temp, function(res) {
            if (res.data.meta.code === '200') {
    ...
    getManagement(id) {
      ...
      this.dialogFormVisible = true
      const _this = this
      require_get('/v1/sbequipmentz/find/' + id, {
        token: token,
        authorityCode: 1
      }, { }, function(res) {
        if (res.data.meta.code === '200') {
      ...
      if (_this.selectlistRow) {
        if (_this.selectlistRow.length !== 0) {
          _this.$confirm('确认要删除么?', '提示', {
            ...
            console.log(ids)
            require_delete('/v1/sbequipmentz/deletes/' + ids, {
              token: token,
              authorityCode: 1
            }, { }, function(res) {
              console.log(res)
