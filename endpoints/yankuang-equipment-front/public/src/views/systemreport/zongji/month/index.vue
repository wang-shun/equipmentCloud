<template>
  <div class="app-container">
    <div class="filter-container">
      <el-date-picker type="year" placeholder="选择年" style="width: 140px;" value-format="yyyy" format="yyyy" v-model="listQuery.year"></el-date-picker>
      <el-date-picker type="month" placeholder="选择月" style="width: 140px;" value-format="M" format="M" v-model="listQuery.month"></el-date-picker>
      <el-select filterable clearable style="width: 140px" class="filter-item" v-model="listQuery.assetComp" placeholder="选择单位" @change="getAssetName">
        <el-option v-for="item in applyCompanyList" :key="item.assetComp" :label="item.name" :value="item.assetComp">
        </el-option>
      </el-select>
    ...
    <div id="reportDiv">
      <!-- 综机折旧修理费报表 -->
      <el-table :data="tableData3" border fit highlight-current-row style="width: 100%;max-height:740px;" :span-method="objectSpanMethod">
        <el-table-column align="center" :label="`${listQuery.year || ''}年${assetName.name ? assetName.name : ''}综机折旧修理费月报---${listQuery.month || ''}月份`">
      ...
      <el-table :data="tableData" :show-header="showHeader" border fit highlight-current-row style="width: 100%;;margin-top:20px" :span-method="objectSpanMethod1">
      ...
      <el-table :data="totalNum" :show-header="showHeader" border fit highlight-current-row style="width: 100%;" :span-method="objectSpanMethod2">
        <el-table-column align="center">
...
export default {
  ...
  methods: {
    ...
    getAssetName(val) { // 获取选择单位的信息
      this.assetName = this.applyCompanyList.find((item) => { // 获取选中的单位code
        return item.assetComp === val
      })
    },
    search() { // 搜索综机折旧修理费报表 方法
      if (this.listQuery.assetComp === '' || this.listQuery.year === '' || this.listQuery.month === '') {
        this.$notify({
          title: '系统提示',
          message: '请完整选择搜索条件！',
          type: 'warning',
        ...
        jsonString.monthTime = month
        jsonString.assetComp = this.listQuery.assetComp // 部门编号code
    ...
    exportExcel() { // 点击导出按钮触发保存
      if (this.tableData3.length !== 0 || this.tableData.length !== 0) {
        ...
        const params = {
          assetComp: this.listQuery.assetComp,
          reportName: `${this.listQuery.year}年${this.assetName.name}综机折旧修理费月报---${this.listQuery.month}月`,
    ...
    reset() {
      ...
      this.listQuery.assetComp = '' // 单位信息
      this.tableData3 = []
      this.tableData = []
    }
  },
  filters: {
    assetCompFilter(value) {
      let name = ''
      value === '1180' ? name = '煤业综机折旧修理费报表（1180）' : name = '东华综机折旧修理费报表（1730）'
      return name
    }
  }
}
