<template>
  <div class="app-container">
    <div class="filter-container1">
      <el-date-picker type="year" v-model="listQuery.year" placeholder="选择年" value-format="yyyy" format="yyyy" style="width: 140px;"></el-date-picker>
    ...
    <div id="reportDiv">
    <!-- 综机折旧修理费报表 -->
      <el-table :data="tableData3" border fit highlight-current-rowstyle="width: 100%;max-height:740px;" :span-method="objectSpanMethod">
        <el-table-column align="center" :label="`${listQuery.year}年${assetName.name || ''}综机折旧修理费报表（${listQuery.assetComp}）`">
      ...
      <el-table :data="tableData" :show-header="showHeader" border fit highlight-current-row style="width: 100%;max-height:740px;margin-top:20px" :span-method="objectSpanMethod1">
      ...
      <el-table :data="totalNum" :show-header="showHeader" border fit highlight-current-row style="width: 100%;max-height:740px" ref="sumTable" :span-method="objectSpanMethod2">
        <el-table-column align="center">
      ...
      <el-table :data="analyseTable" class="fix-header1">
        <el-table-column>
          <template slot-scope="scope">
            <el-input v-model="analyse">
              <template slot="prepend">分析：</template>
            </el-input>
            <span style="display: none">{{analyse}}</span>
					</template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    search() { // 搜索综机折旧修理费报表 方法
      if (this.listQuery.assetComp === '' || this.listQuery.year === '') {
        this.$notify({
          title: '系统提示',
          message: '请完整选择搜索条件！',
          type: 'warning',
    ...
    exportExcel() { // 点击导出按钮触发保存
      if (this.tableData3.length !== 0 || this.tableData.length !== 0) {
        ...
        const params = {
          ...
          reportName: `${this.listQuery.year}年${this.assetName.name}综机折旧修理费报表`,
