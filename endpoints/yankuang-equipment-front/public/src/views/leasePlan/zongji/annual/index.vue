<template>
  <div class="app-container">
    <div class="filter-container">
      <div class="condition">
        <el-date-picker v-model="listQuery.planYear" type="year" :editable="false" value-format="yyyy" format="yyyy" style="width: 200px;" class="filter-item" placeholder="选择年"></el-date-picker>
...
export default {
  ...
  data() {
    return {
      ...
      temp: {
        ...
        positionId: '', // 提出单位
        planRemarks: '', // 备注
        planCreatorId: sessionStorage.getItem('id'), // 创建人Id
  ...
  methods: {
    ...
    getList(page, size) { // 年度计划列表
      ...
      require_post('/v1/elplan/list/integrated/year?pageNum=' + page + '&pageSize=' + size, {
        token: token,
        authorityCode: 1
      }, params, function(res) {
    ...
    addYearPlan() { // 添加年度计划
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          if (eq_arr.length !== 0) {
            require_post('/v1/elplan/integrated/year', {
              token: token,
              authorityCode: 1
            }, params, function(res) {
              if (res.data.status === 200) {
