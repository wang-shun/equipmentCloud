  ...
  <div class="app-container">
    <div class="filter-container">
      <div class="condition">
        ...
        <el-select filterable clearable style="width: 220px" class="filter-item" v-model="listQuery.planPosition" placeholder="提出单位" collapse-tags  multiple @change="selectAll">
      ...
      <div class="btn_right"  v-for='item in authorityCodebutton' :key='item.a'>
        ...
        <el-button class="filter-item" size="small" style="margin-left: 10px;" type="primary" v-waves icon="el-icon-circle-check" v-if="item.name === '提交'" @click="submit()">{{item.name}}</el-button>
        <el-button class="filter-item" size="small" style="margin-left: 10px;" type="primary" v-waves icon="el-icon-question" v-if="item.name === '审核'" @click="examine()">{{item.name}}</el-button>
    ...
    <el-table :key='tableKey' :data="specificationModelTable" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column width="100px" align="center" label="提报人">
        <template slot-scope="scope">
          <span>{{scope.row.planCreatorName}}</span>
        </template>
      </el-table-column>
      <el-table-column width="130px" align="center" label="提报时间">
        <template slot-scope="scope">
          <span>{{scope.row.planCreateTime}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column width="130px" align="center" label="审核时间">
        <template slot-scope="scope">
          <span>{{scope.row.planCreatorName}}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getEqSmallTypeList(event) { // 中类联动设备小类
      ...
      this.getSpModelList('', selectMiddleCode)
    },
    ...
    getSpModelList(event, typeTwo) { // 设备规格型号
      const _this = this
      const that = _this
      const params = {
        sbtypeThree: event,
    ...
    getList(page, size) { // 应急租赁计划列表
      ...
      require_post('/v1/elplan/list/integrated/urgent?pageNum=' + page + '&pageSize=' + size, {
        token: token,
        authorityCode: 1
      }, params, function(res) {
        var data = []
    ...
    addMonthPlan() { // 添加应急租赁计划
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          if (eq_arr.length !== 0) {
            require_post('/v1/elplan/integrated/urgent', {
              token: token,
              authorityCode: 1
            }, params, function(res) {
              if (res.data.status === 200) {
