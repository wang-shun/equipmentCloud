  ...
  <div class="app-container">
    <div class="filter-container">
      <div class="condition">
        ...
        <el-select filterable clearable style="width: 220px" class="filter-item" v-model="listQuery.planMonth" placeholder="选择月" collapse-tags  multiple @change="selectAlly">
          <el-option v-for="item in monthLists" :key="item.code" :label="item.name" :value="item.code">
          </el-option>
        </el-select>
        ...
        <el-select filterable clearable style="width: 220px" class="filter-item" v-model="listQuery.planStatus" placeholder="状态" collapse-tags  multiple @change="selectAllzt">
      ...
      <div class="btn_right" v-for='item in authorityCodebutton' :key='item.a'>
        <el-button class="filter-item" size="small" style="margin-left: 10px;" type="primary" v-waves icon="el-icon-circle-plus-outline" @click="handleCreate" v-if="item.name === '添加'">{{item.name}}</el-button>
        <el-button class="filter-item" size="small" style="margin-left: 10px;" type="primary" v-waves icon="el-icon-edit" @click="edit()" v-if="item.name === '编辑'">{{item.name}}</el-button>
        <el-button class="filter-item" size="small" style="margin-left: 10px;" type="primary" v-waves icon="el-icon-circle-check" @click="submit()" v-if="item.name === '提交'">{{item.name}}</el-button>
        <el-button class="filter-item" size="small" style="margin-left: 10px;" type="primary" v-waves icon="el-icon-question" @click="examine()" v-if="item.name === '审核'">{{item.name}}</el-button>
        <el-button class="filter-item" size="small" style="margin-left: 10px;" type="danger" v-waves icon="el-icon-delete" @click.prevent="delData()" v-if="item.name === '删除'">{{item.name}}</el-button>
      </div>
    </div>

    <!--月度计划列表-->
    <el-table :key='tableKey' :data="specificationModelTable" v-loading="listLoading" border fit highlight-current-row
      style="width: 100%;max-height:740px;" @selection-change='selectRow' ref="moviesTable" :height="tableHeight" :cell-style="statusStyle">
      ...
      <el-table-column width="100px" align="center" label="计划月份">
        <template slot-scope="scope">
          <span class="link-type" align="center" @click="handleInfo(scope.row)">{{scope.row.planMonth | monthFilter}}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="80%">
      ...
      <el-form :rules="rules" status-icon ref="dataForm" :model="temp" label-position="right" label-width="120px" :disabled="setDisabled">
        ...
        <el-form-item label="计划月份" required prop="planMonth" class="el-col el-col-12">
          <el-select filterable class="filter-item" v-model="temp.planMonth" placeholder="选择月" style="width:250px">
        ...
        <el-form-item label="备注" prop="planRemarks">
          <el-input type="textarea" v-model="temp.planRemarks" :maxlength="200">
          </el-input>
        </el-form-item>
      </el-form>
      ...
      <div slot="footer" class="dialog-footer" v-if="isUpdate">
        ...
        <el-button v-if="dialogStatus=='create'" type="primary" @click="addMonthPlan">确定</el-button>
        <el-button v-else type="primary" @click="updateMonthPlan">确定</el-button>
      </div>
...
const monthlabel = monthList.reduce((acc, cur) => { // 计划月份
...
export default {
  ...
  data() {
    ...
    return {
      ...
      specificationModelTable: [], // 月度计划数据集
      ...
      rules: {
        ...
        planMonth: [{ required: true, message: '请选择月份', trigger: 'change' }],
        ...
        equipmentNum: [{ validator: validateInt, msg: '请填写设备数量' }]
      },
  ...
  created() {
    ...
    this.listQuery.planMonth = this.monthDate()
  ...
  methods: {
    ...
    statusStyle({ row, column, rowIndex, columnIndex }) {
      if (row.planStatus === '1' && columnIndex === 5) { // 指定坐标
        return 'color: red'
      } else {
        return ''
      }
    ...
    handleInfo(row) { // 计划详细弹窗
      ...
      this.getMonthPlanInfo(row.planId)
    },
    eqDialogShow() { // 选择设备弹窗
      ...
      require_get('/v1/elplan', {
        ...
        if (res.data.status === 200) {
          ...
          if (pCode === '') {
            ...
            arr.unshift(quan)
            _this.unitList = arr
            _this.proposedCompanylabel = data.reduce((acc, cur) => { // 将返回的id过滤显示相应的name
              acc[cur.id] = cur.name
              return acc
            }, {})
          } else {
    ...
    getList(page, size) { // 应急租赁计划列表
      this.listLoading = true
      const _this = this
      const params = {
        planYears: this.listQuery.planYears,
        planMonths: this.listQuery.planMonth,
        planStatusS: this.listQuery.planStatus,
        planPositions: this.listQuery.planPosition
      }
      require_post('/v1/elplan/list/generic/urgent?pageNum=' + page + '&pageSize=' + size, {
        token: token,
        authorityCode: 1
      }, params, function(res) {
        var data = []
        if (res.data.status === 200) {
          ...
          for (let i = 0; i < result.length; i++) {
            ...
            obj.planMonth = result[i].planMonth
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          const params = {
            ...
            planMonth: this.temp.planMonth
          }
          if (eq_arr.length !== 0) {
            require_post('/v1/elplan/generic/urgent', {
              token: token,
              authorityCode: 1
            }, params, function(res) {
              if (res.data.status === 200) {
      ...
      if (this.selectlistRow) {
        if (this.selectlistRow.length !== 0 && this.selectlistRow.length === 1) {
          if (this.selectlistRow[0].planStatus === '1') {
            ...
            this.getMonthPlanInfo(this.selectlistRow[0].planId)
          } else {
            this.$notify({
              title: '系统提示',
              message: '该计划已提交不可编辑！',
    ...
    getMonthPlanInfo(id) { // 获取月度计划详细
      ...
      require_get('/v1/elplan/' + id, {
        ...
        if (res.data.status === 200) {
          const result = res.data.data
          _this.temp.planYear = result.planYear.toString()
          _this.temp.planId = result.planId
          _this.temp.planMonth = result.planMonth
    ...
    selectAlly(val) {
      const allValues = []
      // 保留所有值
      for (const item of this.monthLists) {
        allValues.push(item.code)
      }
      // 用来储存上一次的值，可以进行对比
      const oldVal = this.oldOptionsy.length === 1 ? this.oldOptionsy[0] : []
      // 若是全部选择
      if (val.includes('全选')) this.listQuery.planMonth = allValues
      // 取消全部选中,上次有当前没有表示取消全选
      if (oldVal.includes('全选') && !val.includes('全选')) this.listQuery.planMonth = []
      // 点击非全部选中,需要排除全部选中以及当前点击的选项
      // 新老数据都有全部选中
      if (oldVal.includes('全选') && val.includes('全选')) {
        const index = val.indexOf('全选')
        val.splice(index, 1) // 排除全选选项
        this.listQuery.planMonth = val
      }
      // 全选未选但是其他选项全部选上,则全选选上,上次和当前,都没有全选
      if (!oldVal.includes('全选') && !val.includes('全选')) {
        console.log(11)
        if (val.length === allValues.length - 1) this.listQuery.planMonth = ['全选'].concat(val)
      }
      // 储存当前最后的结果作为下次的老数据
      this.oldOptionsy[0] = this.listQuery.planMonth
    }
  },
  watch: {
