...
export default {
  ...
  methods: {
      ...
      require_get('/v1/elplan', {
        ...
        if (res.data.status === 200) {
          ...
          if (pCode === '') {
            ...
            // _this.proposedCompanylabel = data_useOrg.reduce((acc, cur) => { // 将返回的id过滤显示相应的name
            //   acc[cur.id] = cur.name
            //   return acc
            // }, {})
          }
    ...
    getList(page, size) { // 月度计划列表
      ...
      require_post('/v1/elplan/list/generic/month?pageNum=' + page + '&pageSize=' + size, {
        token: token,
        authorityCode: 1
      }, params, function(res) {
    ...
    addMonthPlan() { // 添加月度计划
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          if (eq_arr.length !== 0) {
            require_post('/v1/elplan/generic/month', {
              token: token,
              authorityCode: 1
            }, params, function(res) {
              if (res.data.status === 200) {
